...
(function (dltk, window) {
    ...
    dltk.ERROR_NOT_EXIST = 'not exist';
    dltk.HOOK_DEFAULT_SUFFIX = 'Default';
    dltk.HOOK_ASYNC_SUFFIX = 'Async';
    dltk._searchHookfunctions = function _searchHookfunctions(targets, flags, name) {
        // Finds all possible hook functions with the name prefix ``name`` in ``targets``.
        ...
        for (var i = 0; i < targets.length; i++) {
            if (callable(targets[i][name])) {
                ...
                jobs.push(dltk.ERROR_NOT_EXIST);
        }
    };

    dltk._callHookfunctionsSyncOnly = function _callHookfunctionsSyncOnly(targets, flags, name, argdct) {
        var jobs_default = dltk._searchHookfunctions(targets, flags, name + dltk.HOOK_DEFAULT_SUFFIX);
        var jobs = dltk._searchHookfunctions(targets, flags, name);
        var results_default = runJobsSync(jobs_default, argdct);
        var results = runJobsSync(jobs, argdct);
        return {resultsSyncDefault: results_default, resultsSync: results};
    };

    dltk._callHookfunctions = function _callHookfunctions(targets, flags, name, argdct, onFinish) {
        var jobs_async_default = dltk._searchHookfunctions(targets, flags,
                name + dltk.HOOK_ASYNC_SUFFIX + dltk.HOOK_DEFAULT_SUFFIX);
        var jobs_async = dltk._searchHookfunctions(targets, flags, name + dltk.HOOK_ASYNC_SUFFIX);
        ...
        dltk.runJobsAsync(jobs_async_default, argdct, function (results_async_default) {
            ...
            dltk.runJobsAsync(jobs_async, argdct, function (results_async) {
                ...
                var results = _callHookfunctionsSyncOnly(targets, flags, name, argdct);
    ...
    dltk.Experiment = function Experiment(_$, optdct) {
        ...
        var _callHookfunctionsAsync = function _callHookfunctionsAsync(name, argdct, onFinish, idx) {
            // Call all hook functions one by one asynchronously.
            var fn, enabled;
            var i = defined(idx) ? idx : 0;

            if (i < 0 || i > that.modules.length) {   // "i > that.modules.length" is not a mistake
                // finished calling all
                if (callable(onFinish)) onFinish();
                return;
            }

            if (i < that.modules.length) {
                fn = that.modules[i][name];
                enabled = that.modules_isenabled[i];
            }
            else {  // i == that.modules.length
                fn = o[name];
                enabled = true;
            }

            if (!callable(fn) || !enabled) {
                _callHookfunctionsAsync(name, argdct, onFinish, i + 1);
                return;
            }

            if (!defined(argdct) || argdct === null || typeof(argdct) != 'object') argdct = {};
            argdct.finished = function () { _callHookfunctionsAsync(name, argdct, onFinish, i + 1); };
            fn(argdct);
            // ^ this fn() MUST call argdct.finished() upon successful completion of job.
            // Otherwise, the experiment will hang.
        };
        ...
        var _afterPassBenchmark = function _afterPassBenchmark() {
            ...
            if (o.automaticallyRunPreloadAfterPassBenchmark) {
                ...
                _callHookfunctionsAsync('onPreloadRsrcsAsync', argdct, function () {
                    // when finished all async preloading....
                    _callHookfunctions('onPreloadRsrcs', argdct);  // process all synchronous preloading ...
                    _callHookfunctions('onAfterPreloadAllModulesRsrcs');  // ... then call those
                    if (o.automaticallyRunPreBeginExpAfterPreload) that.preBeginExp();  // .. then proceed
                });
            }
        };
        ...
        this.runNextTrial = function runNextTrial() {
            ...
            _callHookfunctionsAsync('onRunNextTrialAsync', argdct, function () {
                // when finished all async trial running stuffs....
                _callHookfunctions('onRunNextTrial', argdct);  // run all synchronous stuffs if any ...
            });
        };
        ...
        this.callModule = function callModule() {
            ...
            //    callModule(module_handle, 'theMethodToCall', args...)
            ...
            //    callModule('theMethodToCall', args...)
            ...
            if (module_handle < 0 || module_handle >= that.modules.length) {
                dltk.setDebugMessage('callModule: Invalid module_handle');
                return null;
            }

            if (!callable(that.modules[module_handle][fn])) {
                dltk.setDebugMessage('callModule: not callable' + String(fn));
                return null;
            }
        ...
        this.callAllModules = function callAllModules(fn, arg) {
            ...
            var res = [];
            for (var i = 0; i < that.modules.length; i++) {
                res.push(that.callModule(i, fn, arg));
            }
            return res;
        };
        ...
        this.setCurrentTrialNumber = function setCurrentTrialNumber(num, module_handle) {
            ...
            return that.callModule(module_handle, 'setCurrentTrialNumber', num);
        };
