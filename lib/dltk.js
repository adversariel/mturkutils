...
// -- Browser detection and screen size related stuff
// references:
...
var dltkBrowserDetect = {
	init: function (optStrictMode) {
        // by default, use strict mode
        var strict = (typeof(optStrictMode) == 'undefined') ? true :  optStrictMode; 
...
dltkBrowserDetect.init();                // by default, use the new "strict" mode, which detects IE11+ correctly
/*ignore jslint start*/ /* jshint ignore:start */
var BrowserDetect = dltkBrowserDetect;   // <- DO NOT REMOVE for compatibility 
/*ignore jslint end*/ /* jshint ignore:end */


// Window dimensions and screen resolution stuffs
function dltkGetScreenAndWindowDimensions(suppressGlobalVar) {
...
// v this causes setting the following global variables: vertical, horizontal, var winW, winH
dltkGetScreenAndWindowDimensions();  


// -- Main part of dicarlo lab js toolkit
(function (dltk, window) {
    ...
    dltk.Experiment = function Experiment(_$, optdct) {
        ...
        var OS = dltkBrowserDetect.OS;
        var BROWSER = dltkBrowserDetect.browser;

        var OPTDCT_DEFAULT = {
            /*** various document elements ***/
            elemSystemmsg: null, elemNotice: null, elemFallback: null,
            elemWarning: null, elemPreload: null,
            elemTutorialLink: null, elemTutorial: null, elemTimer: null, elemBeginTaskGroup: null,
            elemTest: null, elemRespGroup: null, elemBeginTaskBtn: null, elemFPSBench: null,
            elemFPSBenchCanvasID: undefined,  // <- default must be undefined.  DONT PREPEND "#"!!
            /*** various hooker functions ***/
            ...
            supportedBrowser: ['Chrome', 'Firefox'],   // do not add browser that doesn't support performance.now()
            ...
            maxHeightInThisExp: 500,   // max # of pixels needed for this experiment.  Used in recomputeOffset()
        ...
        var $ = function $(elem) {
            ...
            if (target === null || target.length === 0) {
                ...
                if (dbgMessage) window.console.log('NOOP: ' + elem);
        ...
        this.exp_started = false;
        this.aID = dltk.getURLParameter(o.aID);
        this.benchmark_passed = false;
        this.benchmark_finished = false;
        this.benchmark = null;
        this.benchmarks = [];
        this.retry = 0;   // number of benchmark retrial
        ...
        var _afterPassBenchmark = function _afterPassBenchmark() {
            ...
            if (callable(o.onAfterPassBenchmark)) o.onAfterPassBenchmark();
        };
        ...
        var _checkSystem = function _checkSystem(benchmark) {
            ...
            that.benchmark.retry = that.retry;
        ...
        this.beginExp = function beginExp() {
            ...
            $(o.elemPreload).hide();
            $(o.elemNotice).hide();
            if (callable(o.onBeginExp)) o.onBeginExp();  //primeSystemAndRunTrialOnce();
        };
        ...
        this.recomputeOffset = function recomputeOffset() {
            // Recenters few stuffs by using heuristics.
            // This is mainly used to circumvent bad rendetion of position:fixed inside iframe
            var diminfo = dltkGetScreenAndWindowDimensions(true);   // this doesn't update global variables
            ...
            $(o.elemTutorialLink).css('margin-top', offsetToTop + 'px');
            if (that.benchmark_passed) {
                $(o.elemWarning).css('position', 'absolute');
                $(o.elemWarning).css('top', '50%');
                $(o.elemWarning).css('margin-top', offsetToTop + 'px');

                $(o.elemPreload).css('position', 'absolute');
                $(o.elemPreload).css('top', '50%');
                $(o.elemPreload).css('margin-top', offsetToTop + 'px');
            }

            if (callable(o.onRecomputeOffset))
                o.onRecomputeOffset(winW, winH, horizontal, vertical, offsetToTop);  // bad naming convention...
            if (dbgMessage) window.console.log('Resized event detected:' + offsetToTop);
        };

        this.init = function init() {
            // preps variables and do minimal compatibility check
            ...
            if (o.supportedBrowser.indexOf(BROWSER) < 0 || !defined(vertical) || !defined(horizontal)) {
                msg = o.supportedBrowser.join(', ');
                pos = msg.lastIndexOf(', ');
                msg = (pos < 0) ? msg : msg.slice(0, pos) + ' or' + msg.slice(pos + 1);
    ...
    /*
    dltk.RSVPExperiment = function RSVPExperiment(imgFiles, _$, optdct) {
        this.init = function init() {
            // double buffering. from: http://blog.bob.sh/2012/12/double-buffering-with-html-5-canvas.html
            var ctx_test_on = dltk.getOnScreenContextFromCanvas('main_test');
            var ctx_resp0_on = dltk.getOnScreenContextFromCanvas('img4');
            var ctx_resp1_on = dltk.getOnScreenContextFromCanvas('img5');
        };

    };  // end of RSVPExperiment
    */
