 ...
 */
var dltk = {};   // this eliminates closures
// NOTE: It has been decided NOT to use the following fancy form to
// define a name space:
// (function (dltk, window) {
//    ... contents ...
// }(window.dltk = window.dltk || {}, window));
// 
// This decision was made mainly in order to minimize the use of
// closures, since they can cause reduced performance in many time
// critical codes.
...
var callable = dltk.callable;    // internal use only

dltk.defined = function defined(obj) {
    return typeof(obj) != 'undefined';
};
var defined = dltk.defined;      // internal use only
...
dltk.__apply_queue = [];   // functions and arguments to be processed.
dltk.__apply_inner = function __apply_inner() {
    // The next job to be processed: pops the current 0-th element
    var fnargsctx = dltk.__apply_queue.splice(0, 1)[0];
...
dltk._apply = function _apply(fn, args, context) {
    ...
    dltk.__apply_queue.push([fn, args, ctx]);
    setTimeout(dltk.__apply_inner, 0);
};
...
dltk.runJobsAsync = function runJobsAsync(jobs, argdct, onFinish) {
    ...
    if (!defined(argdct) || typeof(argdct) != 'object') {
        ...
        arg = dltk.copydct(src, ['__results__']);
        arg.__protected__ = dltk.copydct(src);
    ...
    if (!defined(_jobs) || _jobs.length === 0) {
        ...
        if (callable(onFinish))
            dltk._apply(onFinish, [arg.__protected__.__results__]);
        return;   // done
    }
    ...
    arg.finished = function _runJobsAsync_inner(resultdct) {
        ...
        dltk._apply(dltk.runJobsAsync, [_jobs, arg, onFinish]);
    };
 ...
 *************************************************************************/
dltk._measureTimeResolution_inner = function _measureTimeResolution_inner(argdct) {
    // this: d, sum_diff, sum_sq_diff, mean_diff, callback
    var o = argdct.__protected__;   // original copy
    var diff = performance.now() - o.d;
    o.sum_diff += diff;
    o.sum_sq_diff += diff * diff;
    if (this.i++ == this.DUR) {
        // done calculating js time resolution
        this.mean_diff = this.sum_diff / this.i;
        dltk.js_tres = Math.max(this.mean_diff, 0);
        dltk.js_tres_variance = this.sum_sq_diff / this.i - this.mean_diff * this.mean_diff;
        if (callable(this.callback)) this.callback(dltk.js_tres, dltk.js_tres_variance);
    }
    else {
        setTimeout(dltk._measureTimeResolution_inner.bind(this), 0);
    }
...
dltk.measureTimeResolution = function measureTimeResolution(callback, duration) {
    ...
    var context = {d: d, sum_diff: sum_diff, sum_sq_diff: sum_sq_diff,
        mean_diff: mean_diff, callback: callback};

    setTimeout(dltk._measureTimeResolution_inner.bind(context), 0);
};

dltk._setTimeout2Cnt = 0;  // debugging purposes only. do not rely on this variable.
dltk._setTimeout2_inner = function _setTimeout2_inner() {
    while(performance.now() - this.t0 < this.delay - dltk.EPS) dltk._setTimeout2Cnt++;
    this.fn();
};
...
dltk.setTimeout2 = function setTimeout2(fn, delay) {
    ...
    var setTimeout2_inner = dltk._setTimeout2_inner.bind({t0: t0, delay: delay, fn: fn});
...
dltk._measureBogoMIPS_inner = function _measureBogoMIPS_inner() {
    if (this.cnt < this.REPS) {
        this.meas.push(dltk.measureBogoMIPSOnce());
        this.cnt++;
        setTimeout(dltk._measureBogoMIPS_inner.bind(this), 0);
        return;
    }
    dltk.bogoMIPS = this.meas.mean();
    if (callable(this.callback)) this.callback(dltk.bogoMIPS);
};
...
dltk.measureBogoMIPS = function measureBogoMIPS(callback, reps) {
    ...
    dltk._measureBogoMIPS_inner.bind({meas: meas, cnt: cnt, REPS: REPS, callback: callback});
};

dltk._measureScreenRefreshIntervals_inner = function _measureScreenRefreshIntervals_inner(ts) {
    var timestamps = this.timestamps;
    dltk.drawToContext('color:' + COLOR, ctx);
    timestamps.push(ts);

    if (timestamps.length >= this.REPS) {
        // finished testing
        dltk.refresh_test_timestamps = timestamps;
        if (callable(this.callback)) setTimeout(function() {
            ...
            var timestamps = dltk.refresh_test_timestamps;   // fu closures
            var isquantized = function (x) { return (dltk.toMicrosec(x) % 100 === 0) ? 1 : 0; };
        ...
        window.requestAnimationFrame(dltk._measureScreenRefreshIntervals_inner.bind(this));
};
...
dltk.prepareResources = function prepareResources(imgFiles, onScrContexts, callback, progressfn, optdct) {
    ...
    var idx = 0;
    var idx_inner = 0;
    var idx_inner2 = 0;   // inner inner index
    ...
    var prepareResources_inner = function prepareResources_inner() {
        ...
        // -- not done
        // if all inner things for this idx are complete...
        if (idx_inner >= imgFiles[idx].length) {
            idx_inner = 0;
            idx_inner2 = 0;
            idx++;
        ...
        // check if imgFiles[idx][idx_inner] is an URL string.
        // if so, roughly speaking, treat it as a length-1 array.
        var sublistEffectiveLen = 0, isURL = false;
        if (typeof(imgFiles[idx][idx_inner]) == 'string') {
            isURL = true;
            sublistEffectiveLen = 1;
        }
        else
            sublistEffectiveLen = imgFiles[idx][idx_inner].length;   // must be an array

        // ...if all elements for this idx_inner are complete
        if (idx_inner2 >= sublistEffectiveLen) {
            idx_inner++;
            idx_inner2 = 0;
            setTimeout(prepareResources_inner, 0);
            return;
        ...
        // Next prepareResources_inner() iteration will be scheduled inside dltk.prepareResourcesOnce().
        if (isURL)  // string
            dltk.prepareResourcesOnce(imgFiles[idx][idx_inner], onScrContexts[idx_inner],
                    prepareResources_inner, optdct);
        else   // list
            dltk.prepareResourcesOnce(imgFiles[idx][idx_inner][idx_inner2], onScrContexts[idx_inner][idx_inner2],
                    prepareResources_inner, optdct);
        idx_inner2++;
