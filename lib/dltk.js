...
(function (dltk, window) {
    ...
    dltk.measureTimeResolution = function measureTimeResolution(callback, duration) {
        ...
        var _measureTimeResolution_inner = function _measureTimeResolution_inner(argdct) {
            argdct.finished(performance.now());
        };

        var _measureTimeResolution_done = function _measureTimeResolution_done(results) {
            var diff = results.diff();
            dltk.js_tres = Math.max(diff.mean(), 0);
            dltk.js_tres_variance = diff.variance();
            if (callable(callback))
                _apply(callback, [dltk.js_tres, dltk.js_tres_variance]);
            callback = null;    // to help GC
        };

        for (var i = 0; i < DUR; i++)
            jobs.push(_measureTimeResolution_inner);

        dltk.runJobsAsync(jobs, {}, _measureTimeResolution_done);
    };
    ...
    dltk.measureBogoMIPS = function measureBogoMIPS(callback, reps) {
        ...
        var _measureBogoMIPS_inner = function _measureBogoMIPS_inner(argdct) {
            argdct.finished(dltk.measureBogoMIPSOnce());
        };

        var _measureBogoMIPS_done = function _measureBogoMIPS_done(results) {
            dltk.bogoMIPS = results.mean();
            if (callable(callback))
                _apply(callback, [dltk.bogoMIPS]);
            callback = null;    // to help GC
        };

        for (var i = 0; i < REPS; i++)
            jobs.push(_measureBogoMIPS_inner);

        dltk.runJobsAsync(jobs, {}, _measureBogoMIPS_done);
    };

    dltk._measureScreenRefreshIntervals_spec = {};
    dltk._measureScreenRefreshIntervals_inner = function _measureScreenRefreshIntervals_inner(ts) {
        var s = dltk._measureScreenRefreshIntervals_spec;
        var timestamps = s.timestamps;
        var callback = s.callback;
        dltk.drawToContext('color:' + s.COLOR, s.ctx);
        timestamps.push(ts);

        if (timestamps.length >= s.REPS) {
            // finished testing
            ...
            if (callable(callback)) {
                ...
                _apply(callback, [timestamps.diff().mean(), timestamps.diff().variance(),
                    quantization_factor, uniqueness_factor], null, 100);
            }
        }
        else
            window.requestAnimationFrame(dltk._measureScreenRefreshIntervals_inner);
    };
    ...
    dltk.measureScreenRefreshInterval = function measureScreenRefreshInterval(canvas, callback, test_color, reps) {
        ...
        dltk._measureScreenRefreshIntervals_spec = {
            REPS: REPS, COLOR: COLOR, ctx: ctx, callback: callback, timestamps: []
        };
    ...
    var qt_tstamps, qt_tstamps_all, qt_tdeltas, /* qt_tdeltas_updates, */ qt_tdeltaflushes;
    var qt_optdct;
    ...
    var qt_render = function qt_render(t) {
        ...
        // var tdeltas_updates = qt_tdeltas_updates;
        var w, h, ctx, url, ctxs, urls, t_jitter, /*t_delay,*/ l, te;
        tstamps.push(t);
        tstamps_all.push(t);

        // -- no need to update the screen
        if (idx >= 0) {
            var statlen = dltk.STATLEN_FPS;
            // half-frame jitter is okay and probably unavoidable
            t_jitter = tstamps_all.slice(-(statlen + 1)).diff().mean() / 2;
            //t_delay = (tdeltas_updates[idx].length < statlen) ? 1 : tdeltas_updates[idx].mean();  // w/ 1ms heuristic
            //t_delay = Math.min(t_delay, t_jitter);
            // dltk._t_compensation = {t_jitter: t_jitter, t_delay: t_delay};    // for diagnostic/debug purposes

            if ((t - t0) < (specs[idx].duration - t_jitter/* - t_delay*/)) {
    ...
    dltk.queueTrial = function queueTrial(specs, callback, optdct) {
        ...
        qt_optdct = dltk.applyDefaults(optdct, optdct_default);
        ...
        /*if (!defined(qt_tdeltas_updates) || qt_tdeltas_updates.length != specs.length) {
            qt_tdeltas_updates = [];
            for (var i = 0; i < specs.length; i++)
                qt_tdeltas_updates.push([]);
            dltk._qt_tdeltas_updates = qt_tdeltas_updates;
        }*/

        if (optdct.measureFlushTiming) {
