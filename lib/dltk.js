...
(function (dltk, window) {
    ...
    dltk.getTimingHistoryExcerpt = function getTimingHistoryExcerpt(history, mode, center, nptsaround) {
        ...
        var excerpt = [];
        var to_us = function (x) { return Math.round(x * 1000); };
        var a;

        if (m == 'diff') {
            var tb = history.flatten()[0];   // the beginning time of this history
            a = history[i_ctr - 1].slice(-n);
            excerpt.push(to_us(tb));
            excerpt.push(to_us(a[0] - tb));  // offset relative to tb
            excerpt.push(a.diff().map(to_us));
            excerpt.push(history[i_ctr].diff().map(to_us));
            excerpt.push(history[i_ctr + 1].slice(0, n).diff().map(to_us));
        }
        else if (m == 'diffeach') {
            a = history[i_ctr - 1].slice(-n);
            excerpt.push(to_us(a[0]));
            excerpt.push(a.diff().map(to_us));
            a = history[i_ctr];
            excerpt.push(to_us(a[0]));
            excerpt.push(a.diff().map(to_us));
            ...
            excerpt.push(history[i_ctr - 1].slice(-n).map(to_us));
            excerpt.push(history[i_ctr].map(to_us));
            excerpt.push(history[i_ctr + 1].slice(0, n).map(to_us));
        }
    ...
    dltk.runBenchmark = function runBenchmark(callback, optdct) {
        ...
        dltk.queueJob(function() {
            if (!defined(canvas_test_fps) || !api_support) {
                dltk.runQueuedJobs();
                return;
            }
            dltk.measureScreenRefreshInterval(canvas_test_fps,
                function(refresh_interval, refresh_interval_variance) {
