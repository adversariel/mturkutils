...
(function (dltk, window) {
    ...
    dltk.JS_TRES_VAR_TOL = 3;         // ~1.7ms deviation deemed fine
    dltk.FRAME_INTERVAL_TOL = 1000 / 60 + 2;   // mean frame interval should be smaller than 16.66ms + 2ms
    dltk.FRAME_INTERVAL_VAR_TOL = 4 * 4;       // jitter shouldn't be larger than 4ms
    ...
    var _measureScreenRefreshIntervals_spec = {};
    var _measureScreenRefreshIntervals_inner = function _measureScreenRefreshIntervals_inner(ts) {
        var s = _measureScreenRefreshIntervals_spec;
        ...
        if (timestamps.length >= s.REPS) {
            ...
            window.requestAnimationFrame(_measureScreenRefreshIntervals_inner);
    };
    ...
    dltk.measureScreenRefreshInterval = function measureScreenRefreshInterval(canvas, callback, test_color, reps) {
        ...
        _measureScreenRefreshIntervals_spec = {
    ...
    dltk.runBenchmark = function runBenchmark(callback, optdct) {
        ...
        bench_jobs.push(function(argdct) {
            ...
            dltk.measureTimeResolution(function (js_tres, js_tres_variance) {
                var x2a = result.js_tres_variance + result.js_tres * result.js_tres;
                var x2b = js_tres_variance + js_tres * js_tres;

                result.js_tres = (js_tres + result.js_tres) / 2;
                result.js_tres_variance = (x2a + x2b) / 2 - result.js_tres * result.js_tres;
                argdct.finished();
            }, tres_dur);
        });
