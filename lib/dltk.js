...
(function (dltk, window) {
    ...
    dltk.Experiment = function Experiment(_$, optdct) {
        ...
        var OPTDCT_DEFAULT = {
            ...
            elemSample: null, elemRespGroup: null, elemBeginTaskBtn: null, elemFPSBench: null,
        ...
        var printDbgMessage = o.printDbgMessage;
        var $ = function $(elem) {
        ...
        this.modules_isactive = [];   // boolean list, where each elem determines if the module is active or not
        ...
        var _callHookfunctions = function _callHookfunctions(name, argdct) {
            // call all hook functions in active modules and optdct
            for (var i = 0; i < that.modules.length; i++)
                if (that.modules_isactive[i] && callable(that.modules[i][name]))
        ...
        this.setDebugMessage = function setDebugMessage(msg) {
            that._debugMsg = msg;
            if (printDbgMessage) window.console.log(msg);
        };
        ...
        this.getCurrentTrialNumbers = function getCurrentTrialNumbers() {
            // get the current trial numbers of all modules
            var nums = [];
            for (var i = 0; i < that.modules.length; i++) {
                if (!callable(that.modules[i].getCurrentTrialNumber))
                    continue;
                nums.push(that.modules[i].getCurrentTrialNumber());
            }
            return nums;
        };
        ...
        this.getTotalTrialNumbers = function getTotalTrialNumbers() {
            // get the total trial numbers of all modules
            var nums = [];
            for (var i = 0; i < that.modules.length; i++) {
                if (!callable(that.modules[i].getTotalTrialNumber))
                    continue;
                nums.push(that.modules[i].getTotalTrialNumber());
            }
            return nums;
        };
        ...
        this.addModule = function addModule(module, optdctmodule) {
            ...
            that.modules.push(m);
            that.modules_isactive.push(true);
            if (that.modules.length != that.modules_isactive.length) {
                that.setDebugMessage('addModule: modules.length != modules_isactive.length');
                return -1;
            }
            // success!
            return handle;
        ...
        this.disableModule = function disableModule(handle) {
            ...
            if (handle < 0 || handle >= that.modules.length) return;
            that.modules_isactive[handle] = false;
        };
        ...
        this.enableModule = function enableModule(handle) {
            ...
            if (handle < 0 || handle >= that.modules.length) return;
            that.modules_isactive[handle] = true;
        };
    ...
    dltk.RSVPModule = function RSVPModule($, optdctmodule, optdctexp, exprobj) {
        // -- Default constants
        ...
        var exp = exprobj;    // reference to the Experiment object
        var primed = false;
        ...
        this.init = function init() {
            ...
            if (m.imgFiles[0][1].length != m.elemTestCanvasIDs.length) {
                exp.setDebugMessage('init: "imgFiles[0][1]" and "elemTestCanvasIDs" have different shapes.');
                return false;
            }
        ...
        this.onAfterPassBenchmark = function onAfterPassBenchmark() {
            // Preload resources.
            ...
            dltk.prepareResources(
                ...
                function() {
                    ...
                    dltk.prepareResources(
                        ...
                        exp.preBeginExp,       // call this when successful
