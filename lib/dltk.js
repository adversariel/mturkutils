...
(function (dltk, window) {
    ...
    dltk.flushCPUCache = function flushCPUCache(callback) {
        var jobs = [];
        var amount_per_iter = 1024 * 1024 / 4 / 8;    // 256k entries by float64
        var n_iter = 20;
        var arr = [];
        ...
        var flushOnce = function flushOnce(argdct) {
            for (var i = 0; i < amount_per_iter; i++)
                arr.push(Math.random());
            argdct.finished();
        };

        var flushOnceCode = function flushOnceCode(argdct) {
            var template = 'abcdef.';
            var template2 = '+*?';
            var l = template.length;
            var l2 = template2.length;
            var re = [];
            var t0 = performance.now();

            for (var i = 0; i < 1024 * 64; i++) {
                for (var j = 0; j < getRandomInt(1, 5); j++)
                    re.push(template[getRandomInt(0, l)]);
                re.push(template2[getRandomInt(0, l2)]);
            }
        ...
        /*for (var j = 0; j < n_iter; j++)
            jobs.push(flushOnce);
        for (var j = 0; j < n_iter; j++)
            jobs.push(flushOnceCode);*/
        jobs.push(function (argdct) { 
            setTimeout(argdct.finished, 3000);
        });

        dltk.runJobsAsync(jobs, {}, function () {
            arr = null;
    ...
    dltk.runBenchmark = function runBenchmark(callback, optdct) {
        ...
        var flush_cpu_cache = defined(optdct) ? optdct.flush_cpu_cache : false;
        ...
        if (flush_cpu_cache)
            bench_jobs.push(function(argdct) {
                dltk.flushCPUCache(function () { argdct.finished(); });
            });
        bench_jobs.push(function(argdct) {
    ...
    var qt_render = function qt_render(t) {
        ...
        if (idx >= 0) {
            if (qt_updated) {
                // there was a screen update in the previous frame
