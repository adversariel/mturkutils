...
(function (dltk, window) {
    ...
    var qt_statlen_fps;
    var qt_updated;
    var qt_t_interval_ideal = 1000 / 60;   // 60 fps
    var qt_bad_transition;
    var qt_tensioner;
    ...
    var qt_render = function qt_render(t) {
        ...
        if (idx >= 0) {
            if (qt_updated) {
                // there was a screen transition in the previous frame
                var n_lost_frame = Math.round((t - t0) / qt_t_interval_ideal - 1.25);
                if (n_lost_frame >= 1) {
                    qt_bad_transition[idx] = {t0: t0, tstamps: tstamps,
                        tdeltas: tdeltas, tdeltaflushes: tdeltaflushes};
                    qt_t0 = t0 = t;
        ...
        // finished?
        if (idx >= specs.length) {
            if (callable(qt_callback)) qt_callback(history, history_delta, history_delta_flush, qt_bad_transition);
            qt_running = false;
            return;
        }

        // do necessary setups prior to draw idx-th component
        if (callable(this_spec.pre)) this_spec.pre(history, history_delta, history_delta_flush, qt_bad_transition);
        ...
        if (callable(this_spec.post)) this_spec.post(history, history_delta, history_delta_flush, qt_bad_transition);
    ...
    dltk.queueTrial = function queueTrial(specs, callback, optdct) {
        ...
        if (qt_optdct.useInTrialTensioning) {
            qt_tensioner = function (t_spent) {
                var t_delay = 5 - t_spent;
                var a, t1 = performance.now();
                while(performance.now() - t1 < t_delay) {
                    for (a = 0; a < 250; a++);
                }
                //console.log('!! ' + a + ' ' + t_delay);
            };
        }
        else
            qt_tensioner = dltk.nop;
