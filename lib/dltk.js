...
(function (dltk, window) {
    ...
    var qt_render = function qt_render(t) {
        ...
        var w, h, ctx, url, ctxs, urls, t_interval, l, te, n_lost_frames;
        tstamps.push(t);
        tstamps_all.push(t);
        t_interval = tstamps_all.slice(-(dltk.STATLEN_FPS + 1)).diff().mean();
        if (t_interval < 1) t_interval = qt_t_interval_default;
        ...
        tdeltas.push(te - t);
        n_lost_frames = Math.round((te - t) / t_interval - 0.25);
        if (n_lost_frames >= 1) {
            qt_t0 = t0 = t + n_lost_frames * t_interval;
            qt_tstamps = [t0];
            console.log('Lost: ' + n_lost_frames + ' at ' + idx);
        }
        window.requestAnimationFrame(qt_render);
        qt_measureFlushTiming();
    };
    ...
    dltk.queueTrial = function queueTrial(specs, callback, optdct) {
        ...
        qt_t_interval_default = (dltk.refresh_interval === null) ? 100 / 6 : dltk.refresh_interval;
