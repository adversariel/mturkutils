...
# various
METAPATH = 'references/meta64.pkl'
...
ADJS = ['light', 'bulbous', 'boxy', 'curly', 'globular', 'disc-like',
    ...
    'cuddly']
S3PREFIX_FULLOBJT = 'https://s3.amazonaws.com/objectome32_final/'
S3BUCKET_FULLOBJT = 'objectome32_final'
S3BUCKET = 'objectome_adjectives_slider_fullvar'


# -- main funcs
def prep(n_reps=50, n_trials_per_chunk=100, with_repl=False, same_across=True,
        rseed=0, nc_objs=64, nc_imgs=1000):
    ...
    for obj in models64:
        ...
        for adj in ADJS:
            ...
            for i in si:
                ...
                imgurl = S3PREFIX_FULLOBJT + meta[i]['id'] + '.png'
                trials.append([imgurl, adj])
                ids.append(meta[i]['id'] + '.png')

    assert len(ids) == n_reps * nc_objs * len(ADJS)
    ids = np.unique(ids)
    assert len(ids) == n_reps * nc_objs
